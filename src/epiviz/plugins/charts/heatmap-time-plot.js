 ...
 */
epiviz.plugins.charts.HeatmapTimePlot.prototype = epiviz.utils.mapCopy(
  epiviz.plugins.charts.HeatmapPlot.prototype
);
epiviz.plugins.charts.HeatmapTimePlot.constructor =
  epiviz.plugins.charts.HeatmapTimePlot;
 ...
 * @private
 */
epiviz.plugins.charts.HeatmapTimePlot.prototype._drawCells = function (
  range,
  data,
  colOrder
) {
  ...
  var maxColumns = this.customSettingsValues()[
    epiviz.plugins.charts.HeatmapTimePlotType.CustomSettings.MAX_COLUMNS
  ];
  ...
  var cluster = this.customSettingsValues()[
    epiviz.plugins.charts.HeatmapTimePlotType.CustomSettings.CLUSTER
  ];
  var clusterRows =
    cluster == epiviz.plugins.charts.HeatmapTimePlotType.Cluster.ROWS ||
    cluster == epiviz.plugins.charts.HeatmapTimePlotType.Cluster.BOTH;
  var clusterCols =
    (cluster == epiviz.plugins.charts.HeatmapTimePlotType.Cluster.COLS ||
      cluster == epiviz.plugins.charts.HeatmapTimePlotType.Cluster.BOTH) &&
    maxColumns >= nEntries;

  var dendrogramRatio =
    this.customSettingsValues()[
    epiviz.plugins.charts.HeatmapTimePlotType.CustomSettings.SHOW_DENDROGRAM
  ...
  data.foreach(function (m, series) {
    ...
    featureValues._values.forEach(function (valData) {
      ...
      if (valData.length > numTimePoints) {
  ...
  // console.log("min, max, numTimepoints");
  // console.log(dataMin);
  // console.log(dataMax);
  // console.log(numTimePoints);
  ...
  var selection = itemsGroup
    .selectAll(".spline-container")
    .data(items, function (d) {
      return d.id;
    });

  var all_features_std_devs = JSON.parse(
    items[0].valueItems[0][0].rowItem._parent._metadata.std_devs
  );
  var all_features_means = JSON.parse(
    items[0].valueItems[0][0].rowItem._parent._metadata.means
  );
  var all_subjects_metadata = JSON.parse(
    items[0].valueItems[0][0].rowItem._parent._metadata.subject_metadata
  );

  var spline_color_field = this.customSettingsValues()[
  ...
  var splinesselection = selection
    ...
    .attr("transform", function (d) {
      return (
        "translate(" +
        cellWidth * colIndex[d.valueItems[0][0].globalIndex] +
        "," +
        cellHeight * d.seriesIndex +
        ")"
      );
    });
  ...
  splinesselection
    ...
    .style("fill", function (d) {
      ...
      label_mean.forEach(function (valueElement, j) {
        greater_than_one_away[j] =
          valueElement + Math.log2(label_std_dev[j] + 1);
      });
  ...
  // self._svg.select("defs").selectAll("linearGradient").remove();
  var gradients = self._svg.select("defs").selectAll("linearGradient");

  gradients.remove();

  gradients.data(items).enter()
  ...
  splinesselection
    ...
    .attr("d", function (d, i) {
      ...
      var xaxis = d3.scale
        .linear()
        .domain(d3.extent([0, numTimePoints - 1]))
        .range([0, cellWidth - 10]);

      var yaxis = d3.scale
        .linear()
        .domain(d3.extent([dataMin - dataMax / 2, dataMax]))
        .range([cellHeight * 0.9, 0]);
...
epiviz.plugins.charts.HeatmapTimePlot.prototype.transformData = function (
  range,
  data
) {
...
epiviz.plugins.charts.HeatmapTimePlot.prototype._applyLogTransformation = function (
  lData,
  callback
) {
  callback(lData);
 ...
 * @private
 */
epiviz.plugins.charts.HeatmapTimePlot.prototype._drawLabels = function (
  itemsGroup,
  colnames,
  colids,
  columnMap,
  ...
  if (colnames.length > nCols) {
    ...
    $("#" + this.id() + " .col-text").each(function (i) {
  ...
  $("#" + this.id() + " .chart-title").each(function (i) {
