...
goog.provide("epiviz.plugins.charts.GuideTrack");
 ...
 * @constructor
 */
epiviz.plugins.charts.GuideTrack = function(id, container, properties) {
 ...
 */
epiviz.plugins.charts.GuideTrack.prototype = epiviz.utils.mapCopy(
  epiviz.ui.charts.Track.prototype
);
epiviz.plugins.charts.GuideTrack.constructor = epiviz.plugins.charts.GuideTrack;

/**
 * @protected
 */
epiviz.plugins.charts.GuideTrack.prototype._initialize = function() {
  // Call super
 ...
 */
epiviz.plugins.charts.GuideTrack.prototype.draw = function(
  range,
  data,
  slide,
  zoom
 ...
 * @private
 */
epiviz.plugins.charts.GuideTrack.prototype._drawGuides = function(
  range,
  data,
  slide,
  zoom
  ...
  if(series.size() <= numGuide) {
    ...
    for (var i=0; i < numGuide; i++) {
      var tempRange = epiviz.datatypes.GenomicRange.fromStartEnd(
        ...
        self.GuidesToShow.push(first_guide.rowItem.metadata("sgrna_id"));
    }
  }
  ...
  var dataItems = indices.map(function(i) {
    ...
    if (showGuides == "default") {
      var classes = sprintf("item guide-%s", item.metadata("sgrna_id"));
 ...
 * @private
 */
epiviz.plugins.charts.GuideTrack.prototype._translateGuide = function(
  elem,
  d,
  delta
) {
...
epiviz.plugins.charts.GuideTrack.prototype.getStacky = function(guideStart, guideEnd, strand,
    itree, maxStack) {
 ...
 * @private
 */
epiviz.plugins.charts.GuideTrack.prototype._drawGuide = function(
  elem,
  d,
  i,
  xScale,
  ...
  var sites_position = [parseInt(rowItem.metadata("pam_site")), parseInt(rowItem.metadata("cut_site"))];
  ...
  sites
    ...
    .append("rect")
    .attr("x", function(j) {
      return xScale(sites_position[j]);
    })
    .attr("y", function(j) {
      var istack = stacky;
    ...
    .attr("width", function(j) {
      if (j == 0) {
        if (rowItem.strand() == "+") {
          return xScale(sites_position[j] + 1) - xScale(sites_position[j]);
        } else {
          return xScale(sites_position[j] + 2) - xScale(sites_position[j]);
        }
      } else {
        return xScale(sites_position[j] + 1) - xScale(sites_position[j]);
      }
 ...
 */
epiviz.plugins.charts.GuideTrack.prototype.colorLabels = function() {
  return ["Guide", "Sites"];
};
...
*/
epiviz.plugins.charts.GuideTrack.prototype.doHover = function(selectedObject) {
