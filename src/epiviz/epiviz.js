 ...
 * @private
 */
epiviz.EpiViz.prototype._addChart = function(
  type,
  visConfigSelection,
  chartId,
  chartProperties
) {
  chartId = this._chartManager.addChart(
    type,
    visConfigSelection,
    chartId,
    chartProperties
  ...
  if (type.typeName() == "epiviz.plugins.charts.CustomScatterPlot") {
    ...
    this._dataManager.getPCA(range, chartMeasurementsMap, function(
      chartId,
      data
    ) {
    ...
    var chartMeasurementsMap = {}
    chartMeasurementsMap[chartId] = visConfigSelection.measurements
    this._dataManager.getDiversity(range, chartMeasurementsMap, function(
      chartId,
      data
    ) {
      self._chartManager.updateCharts(range, data, [chartId])
    ...
    var chartMeasurementsMap = {}
    chartMeasurementsMap[chartId] = visConfigSelection.measurements
    this._dataManager.getData(range, chartMeasurementsMap, function(
      chartId,
      data
    ) {
      self._chartManager.updateCharts(range, data, [chartId])
...
epiviz.EpiViz.prototype._registerRequestSeqInfos = function() {
  var self = this
  this._locationManager.onRequestSeqInfos().addListener(
...
epiviz.EpiViz.prototype._registerRequestMeasurements = function() {
  var self = this
  this._measurementsManager.onRequestMeasurements().addListener(
    new epiviz.events.EventListener(function() {
      self._dataManager.getMeasurements(
...
epiviz.EpiViz.prototype._registerRequestWorkspaces = function() {
  var self = this
  this._workspaceManager.onRequestWorkspaces().addListener(
    new epiviz.events.EventListener(
      /**
       * @param {{activeWorkspaceId: string}} e
       */
      function(e) {
        var cookieWorkspace = self._cookieManager.getWorkspace(
          self._chartFactory,
          self._config
        )
        self._dataManager.getWorkspaces(
          function(rawWorkspaces) {
            var ws = []
            var activeWorkspace = null
            var unchangedActiveWorkspace = null
            for (var i = 0; i < rawWorkspaces.length; ++i) {
              var w = epiviz.workspaces.Workspace.fromRawObject(
                rawWorkspaces[i],
                self._chartFactory,
                self._config
              )
              ...
              if (w.id() == e.activeWorkspaceId) {
                if (
                  cookieWorkspace &&
                  cookieWorkspace.id() == e.activeWorkspaceId
                ) {
            ...
            self._workspaceManager.updateWorkspaces(
              ws,
              activeWorkspace,
              e.activeWorkspaceId,
              unchangedActiveWorkspace
            )
            if (!cookieWorkspace) {
...
epiviz.EpiViz.prototype._registerUiAddChart = function() {
  var self = this
  this._controlManager.onAddChart().addListener(
    new epiviz.events.EventListener(
...
epiviz.EpiViz.prototype._registerUiSaveWorkspace = function() {
  var self = this
  this._controlManager.onSaveWorkspace().addListener(
    new epiviz.events.EventListener(
      /**
      ...
      function(e) {
        ...
        if (workspace) {
          workspace = self._workspaceManager
            .activeWorkspace()
            .copy(e.name, workspace.id())
        } else {
...
epiviz.EpiViz.prototype._registerUiDeleteActiveWorkspace = function() {
  var self = this
  this._controlManager.onDeleteActiveWorkspace().addListener(
    new epiviz.events.EventListener(function() {
      self._dataManager.deleteWorkspace(
        self._workspaceManager.activeWorkspace()
      )
...
epiviz.EpiViz.prototype._registerUiRevertActiveWorkspace = function() {
  var self = this
  this._controlManager.onRevertActiveWorkspace().addListener(
    new epiviz.events.EventListener(function() {
...
epiviz.EpiViz.prototype._registerUiLoginLinkClicked = function() {
  var self = this
  this._controlManager.onLoginLinkClicked().addListener(
    new epiviz.events.EventListener(function() {
...
epiviz.EpiViz.prototype._registerUiSearchWorkspaces = function() {
  var self = this
  this._controlManager.onSearchWorkspaces().addListener(
    new epiviz.events.EventListener(
      /**
...
epiviz.EpiViz.prototype._registerUiActiveWorkspaceChanged = function() {
  var self = this
  this._controlManager.onActiveWorkspaceChanged().addListener(
    new epiviz.events.EventListener(
      /**
          ...
          if (e.newValue.id && !self._workspaceManager.get(e.newValue.id)) {
            ...
            self._dataManager.getWorkspaces(
              function(rawWorkspaces) {
                ...
                if (result) {
                  self._workspaceManager.changeActiveWorkspace(
                    e.newValue.id,
                    result
                  )
                }
            ...
            self._workspaceManager.changeActiveWorkspace(e.newValue.id)
          }
        }

        if (
          epiviz.workspaces.UserManager.USER_STATUS.loggedIn &&
          !self._workspaceManager.activeWorkspaceChanging() &&
          self._workspaceManager.activeWorkspace().changed()
        ) {
...
epiviz.EpiViz.prototype._registerUiSearch = function() {
  var self = this
  this._controlManager.onSearch().addListener(
    new epiviz.events.EventListener(
      /**
...
epiviz.EpiViz.prototype._registerChartRequestHierarchy = function() {
  var self = this
  this._chartManager.onChartRequestHierarchy().addListener(
...
epiviz.EpiViz.prototype._registerChartPropagateHierarchySelection = function() {
  var self = this
  this._chartManager.onChartPropagateHierarchyChanges().addListener(
    new epiviz.events.EventListener(function(e) {
      var map = {}
      map[e.id] = e.args
...
epiviz.EpiViz.prototype._registerChartPropagateNavigationChanges = function() {
  var self = this
  this._chartManager.onChartPropagateNavigationChanges().addListener(
    new epiviz.events.EventListener(function(e) {
...
epiviz.EpiViz.prototype._registerUiSettingsChanged = function() {
  var self = this
  this._workspaceManager.onUiChartSettingsChanged().addListener(
    new epiviz.events.EventListener(function(e) {
      //do nothing!
...
epiviz.EpiViz.prototype._registerDataAddMeasurements = function() {
  var self = this
  this._dataManager.onRequestAddMeasurements().addListener(
    new epiviz.events.EventListener(
...
epiviz.EpiViz.prototype._registerDataRemoveMeasurements = function() {
  var self = this
  this._dataManager.onRequestRemoveMeasurements().addListener(
    new epiviz.events.EventListener(
      /** @param {{measurements: epiviz.measurements.MeasurementSet, result: epiviz.events.EventResult}} e */
      function(e) {
        try {
...
epiviz.EpiViz.prototype._registerDataLoadMeasurements = function() {
  var self = this
  this._dataManager.onRequestLoadMeasurements().addListener(
    new epiviz.events.EventListener(
      /** @param {{measurements: epiviz.measurements.MeasurementSet, result: epiviz.events.EventResult}} e */
      function(e) {
        try {
...
epiviz.EpiViz.prototype._registerDataAddChart = function() {
  var self = this
  this._dataManager.onRequestAddChart().addListener(
    new epiviz.events.EventListener(
...
epiviz.EpiViz.prototype._registerDataRemoveChart = function() {
  var self = this
  this._dataManager.onRequestRemoveChart().addListener(
    new epiviz.events.EventListener(
      /**
...
epiviz.EpiViz.prototype._registerPrintWorkspace = function() {
  var self = this
  this._dataManager.onRequestPrintWorkspace().addListener(
    new epiviz.events.EventListener(
      /**
       * @param {{id: string, result: epiviz.events.EventResult}} e
       */
...
epiviz.EpiViz.prototype._registerLoadWorkspace = function() {
  var self = this
  this._dataManager.onRequestLoadWorkspace().addListener(
    new epiviz.events.EventListener(
      /**
       * @param {{id: string, result: epiviz.events.EventResult}} e
       */
      function(e) {
        try {
          self._workspaceManager._requestWorkspaces.notify({
            activeWorkspaceId: e.workspace,
          })
        } catch (error) {
          e.result.success = false
          e.errorMessage = error.toString()
...
epiviz.EpiViz.prototype._registerDataSetChartSettings = function() {
  var self = this
  this._dataManager.onRequestSetChartSettings().addListener(
    new epiviz.events.EventListener(
      /**
...
epiviz.EpiViz.prototype._registerDataGetChartSettings = function() {
  var self = this
  this._dataManager.onRequestGetChartSettings().addListener(
    new epiviz.events.EventListener(
      /**
       * @param {{id: string, settings: Array, result: epiviz.events.EventResult}} e
       */
...
epiviz.EpiViz.prototype._registerDataAddSeqInfos = function() {
  var self = this
  this._dataManager.onRequestAddSeqInfos().addListener(
    new epiviz.events.EventListener(
      /**
...
epiviz.EpiViz.prototype._registerDataRemoveSeqNames = function() {
  var self = this
  this._dataManager.onRequestRemoveSeqNames().addListener(
    new epiviz.events.EventListener(
      /**
...
epiviz.EpiViz.prototype._registerDataNavigate = function() {
  var self = this
  this._dataManager.onRequestNavigate().addListener(
    new epiviz.events.EventListener(
      /**
...
epiviz.EpiViz.prototype._registerDataRedraw = function() {
  var self = this
  this._dataManager.onRequestRedraw().addListener(
    new epiviz.events.EventListener(
      /**
...
epiviz.EpiViz.prototype._registerDataGetCurrentLocation = function() {
  var self = this
  this._dataManager.onRequestCurrentLocation().addListener(
    new epiviz.events.EventListener(
      /**
       * @param {{result: epiviz.events.EventResult}} e
       */
      function(e) {
        try {
          var currentLocation = self._locationManager.currentLocation()
          e.result.value = {
            seqName: currentLocation.seqName(),
...
epiviz.EpiViz.prototype._registerWorkspacesLoaded = function() {
  var self = this
  this._workspaceManager.onWorkspacesLoaded().addListener(
    new epiviz.events.EventListener(
      /**
       * @param {{
...
epiviz.EpiViz.prototype._registerActiveWorkspaceChanged = function() {
  var self = this
  this._workspaceManager.onActiveWorkspaceChanged().addListener(
    new epiviz.events.EventListener(
      /**
      ...
      function(e) {
        ...
        self._controlManager.updateSelectedWorkspace({
          id: e.newValue.id(),
          name: e.newValue.name(),
        })
        self._locationManager.changeCurrentLocation(e.newValue.range())

        self._measurementsManager.removeMeasurements(
          self._measurementsManager.computedMeasurements()
        )
        self._measurementsManager.addMeasurements(
          e.newValue.computedMeasurements()
        )
        ...
        for (var displayType in charts) {
          if (!charts.hasOwnProperty(displayType)) {
            continue
          }

          for (var i = 0; i < charts[displayType].length; ++i) {
            self._addChart(
              charts[displayType][i].type,
              charts[displayType][i].properties.visConfigSelection,
              charts[displayType][i].id,
              charts[displayType][i].properties.copy()
            )
          }
        }
...
epiviz.EpiViz.prototype._registerActiveWorkspaceContentChanged = function() {
  var self = this
  this._workspaceManager.onActiveWorkspaceContentChanged().addListener(
    new epiviz.events.EventListener(
      /**
...
epiviz.EpiViz.prototype._registerLocationChanged = function() {
  var self = this
  this._locationManager.onCurrentLocationChanged().addListener(
    new epiviz.events.EventListener(
      /**
      ...
      function(e) {
        ...
        for (var mea in chartMeasurementsMap) {
          if (
            mea.indexOf("pca_scatter") != -1 ||
            mea.indexOf("diversity_scatter") != -1
          ) {
            delete chartMeasurementsMap[mea]
          }
        }

        self._dataManager.getData(e.newValue, chartMeasurementsMap, function(
          chartId,
          data
        ) {
        ...
        if (
          !self._chartManager.onChartPropogateIcicleLocationChanges().isFiring()
        ) {
...
epiviz.EpiViz.prototype._registerChartPropogateIcicleLocationChange = function() {
  ...
  self._chartManager.onChartPropogateIcicleLocationChanges().addListener(
    new epiviz.events.EventListener(function(e) {
      //console.log(e);
      var currentLocation = self._locationManager.currentLocation()
      if (currentLocation != null) {
        self._locationManager.changeCurrentLocation(
          new epiviz.datatypes.GenomicRange(
...
epiviz.EpiViz.prototype._registerDataUiStatus = function() {
  var self = this
  this._dataManager.onRequestUiStatus().addListener(
    new epiviz.events.EventListener(
      /**
       * @param {{id: string, settings: Array, result: epiviz.events.EventResult}} e
       */
      function(e) {
        if (self._workspaceManager._activeWorkspace == null) {
          self._workspaceManager.onActiveWorkspaceChanged().addListener(
            new epiviz.events.EventListener(function(e1) {
              e.result.success = true
            })
          )
        } else {
