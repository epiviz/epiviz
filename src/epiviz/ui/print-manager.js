...
epiviz.ui.PrintManager = function(ctrId, fName, fType, workspaceId) {
     ...
     * @type {string}
     */
    this._fName = fName ? fName : "metaviz_" + Math.floor($.now() / 1000);
...
epiviz.ui.PrintManager.prototype.print = function() {
    ...
    function custom_styles(dom) {
        ...
        if(epiviz.utils.stringContains($(dom).attr('id'), "track-lines")) {
        ...
        // update text on heatmaps when 
        var rowTitles = $(dom).find('.row-legend');
        rowTitles.each(function() {
            $(this).attr("x", "-50");
            $(this).css({"font-size": "11px"});
        });
    }
