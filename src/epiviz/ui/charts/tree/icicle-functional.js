...
goog.provide('epiviz.ui.charts.tree.IcicleFunctional');
 ...
 * @constructor
 * @extends {epiviz.ui.charts.tree.Icicle}
 */
epiviz.ui.charts.tree.IcicleFunctional = function(id, container, properties) {

  epiviz.ui.charts.tree.Icicle.call(this, id, container, properties);
  ...
  // /**
  //  * Size of icons on nodes. This should be the same as the font size of ".IcicleFunctional .icon" in svg.css
  //  * @type {number}
  //  * @private
  //  */
  // this._iconSize = 16;
 ...
 */
epiviz.ui.charts.tree.IcicleFunctional.prototype = epiviz.utils.mapCopy(epiviz.ui.charts.tree.Icicle.prototype);
epiviz.ui.charts.tree.IcicleFunctional.constructor = epiviz.ui.charts.tree.Icicle;

/**
 * Initializes the chart and draws the initial SVG in the container
 * @protected
 */
epiviz.ui.charts.tree.IcicleFunctional.prototype._initialize = function() {
  epiviz.ui.charts.tree.Icicle.prototype._initialize.call(this);

  // this._svg.classed('IcicleFunctional', true);
 ...
 */
epiviz.ui.charts.tree.IcicleFunctional.prototype.draw = function(range, root) {
  ...
  if (root) {
    ...
    this._uiData.forEach(function(node) {
        node.selectionType = epiviz.ui.charts.tree.HierarchyVisualization.ENUM_SELECTIONS['REMOVED'];
    });
  ...
  var hoverOpacity = this.customSettingsValues()[epiviz.ui.charts.tree.IcicleFunctionalType.CustomSettings.HOVER_OPACITY];
  var aggLevel = this.customSettingsValues()[epiviz.ui.charts.tree.IcicleFunctionalType.CustomSettings.AGG_LEVEL];
  var nodeSel = this.customSettingsValues()[epiviz.ui.charts.tree.IcicleFunctionalType.CustomSettings.NODE_SEL];
  var IcicleFunctionalRoot = this.customSettingsValues()[epiviz.ui.charts.tree.IcicleFunctionalType.CustomSettings.IcicleFunctional_ROOT];
  var IcicleFunctionalAutoPropagate = this.customSettingsValues()[epiviz.ui.charts.tree.IcicleFunctionalType.CustomSettings.AUTO_PROPAGATE];
  ...
  // this._xScale = d3.scale.linear().range([this._rowCtrlWidth, width - this.margins().sumAxis(Axis.X) - this._rowAllLevelWidth]);
  // this._yScale = d3.scale.linear().range([this._rowCtrlWidth, width - this.margins().sumAxis(Axis.Y) - this._rowAllLevelWidth]);

  //this._yScale = d3.scale.pow().exponent(1.25).range([0, height - this.margins().sumAxis(Axis.Y)]);
  // this._yScale = d3.scale.linear().range([0, height - this.margins().sumAxis(Axis.Y)]);
  // this._xScale = d3.scale.linear().range([0, height - this.margins().sumAxis(Axis.X)]);
  // this._svg.attr('transform', sprintf('translate(0,0) rotate(90)'));
  ...
  var newItems = items
    ...
    .attr('class', function(d) {
      ...
      return 'item ' + epiviz.ui.charts.tree.HierarchyVisualization.SELECTION_CLASSES[0];
    })
    .on('click', function(d) {
      ...
      if (self.selectMode()) {
        ...
        self._customSettingsValues["IcicleFunctionalRoot"] = d.id;
        ...
        self.onRequestHierarchy().notify(new epiviz.ui.charts.VisEventArgs(
          ...
          var IcicleFunctionalAutoPropagate = self.customSettingsValues()[epiviz.ui.charts.tree.IcicleFunctionalType.CustomSettings.AUTO_PROPAGATE];
          
        if(IcicleFunctionalAutoPropagate) {
  ...
  var newIcons = newItems.append('svg:foreignObject')
    ...
    .append('xhtml:span')
    .attr('class', 'unselectable-text fa fa-plus')
    .on('mouseover', function(d) {
    ...
    .on('click', function(d) {
      ...
      console.log(self);
      var newSelectionType = self.selectNode(self._uiDataMap[d.id]);
      self._uiDataMap[d.id].selectionType = newSelectionType;

      // var rdis = "", adis = "", edis = "";
      // if(nodeSelectionType == epiviz.ui.charts.tree.Icicle.ENUM_SELECTIONS['REMOVED'] || nodeSelectionType == epiviz.ui.charts.tree.Icicle.ENUM_SELECTIONS['REMOVED_PRIME']) {
      //   rdis = "disabled";
      // }
      // else if(nodeSelectionType == epiviz.ui.charts.tree.Icicle.ENUM_SELECTIONS['AGGREGATED'] || nodeSelectionType == epiviz.ui.charts.tree.Icicle.ENUM_SELECTIONS['AGGREGATED_PRIME']) {
      //   adis = "disabled";
      // }
      // else if(nodeSelectionType == epiviz.ui.charts.tree.Icicle.ENUM_SELECTIONS['EXPANDED']) {
      //   edis = "disabled";
      // }
      ...
      //     d.selectionType = node.selectionType = self.selectNode(node, epiviz.ui.charts.tree.Icicle.ENUM_SELECTIONS['REMOVED']);
      ...
      //     var node = self._getNewNode(d);
      //     node.selectionType = nodeSelectionType;
      //     d.selectionType = node.selectionType = self.selectNode(node, epiviz.ui.charts.tree.Icicle.ENUM_SELECTIONS['AGGREGATED']);
      //     self._customSettingsValues["nodeSel"] = JSON.stringify(self._selectedNodes);
      //     self._customSettingsChanged.notify(new epiviz.ui.charts.VisEventArgs(self._id, self._customSettingsValues));
      //   }
      ...
      //     var node = self._getNewNode(d);
      //     node.selectionType = nodeSelectionType;
      //     d.selectionType = node.selectionType = self.selectNode(node, epiviz.ui.charts.tree.Icicle.ENUM_SELECTIONS['EXPANDED']);
      //     self._customSettingsValues["nodeSel"] = JSON.stringify(self._selectedNodes);
      //     self._customSettingsChanged.notify(new epiviz.ui.charts.VisEventArgs(self._id, self._customSettingsValues));
      //   }
  ...
  itemsGroup.selectAll('.item').selectAll('rect')
    ...
    .style('fill', function(d) { 
      console.log(d);
      //TODO - The d.selectionType is 1 by default, so opposite
      if(d.selectionType == epiviz.ui.charts.tree.HierarchyVisualization.ENUM_SELECTIONS['REMOVED']){
  ...
  Object.keys(self._selectedNodes).forEach(function(sel) {
       if(self._uiDataMap[sel]) {
         self.selectNode(self._uiDataMap[sel]);
       }
  });
  ...
  if(this._firstRun == 0) {
    ...
    self.onRequestHierarchy().notify(new epiviz.ui.charts.VisEventArgs(
      self.id(),
      new epiviz.ui.controls.VisConfigSelection(undefined, undefined, self.datasourceGroup(), self.dataprovider(), undefined, undefined, undefined, IcicleFunctionalRoot))
    );
...
epiviz.ui.charts.tree.IcicleFunctional.prototype._updateChartLocation = function(start, width) {

  var self = this;
        
  self._propagateIcicleFunctionalLocationChanges.notify({start: start, width: width});
...
epiviz.ui.charts.tree.IcicleFunctional.prototype._updateLocation = function(start, width) {
  var self = this;
  self._lastRange = new epiviz.datatypes.GenomicRange('NA', 
                          start, 
                          width);
...
epiviz.ui.charts.tree.IcicleFunctional.prototype._drawAxes = function(root) {
 ...
 * @private
 */
epiviz.ui.charts.tree.IcicleFunctional.prototype._drawRowControls = function(root) {
  var self = this;
 ...
 */
epiviz.ui.charts.tree.IcicleFunctional.prototype.doHover = function(selectedObject) {
...
/**
 */
epiviz.ui.charts.tree.IcicleFunctional.prototype.doUnhover = function() {
 ...
 */
epiviz.ui.charts.tree.IcicleFunctional.prototype.doSelect = function(selectedObject) {
  var itemsGroup = this._svg.select('.items');
  var selectItems = itemsGroup.selectAll('.item').filter(function(d) {
...
/**
 */
epiviz.ui.charts.tree.IcicleFunctional.prototype.doDeselect = function() {
  this._svg.select('.items').selectAll('.selected').classed('selected', false);
};

epiviz.ui.charts.tree.IcicleFunctional.prototype.hoverHierarchy = function(selectedObject) {
...
epiviz.ui.charts.tree.IcicleFunctional.prototype.notifyAggregateNode = function(node) {
 ...
 */
epiviz.ui.charts.tree.IcicleFunctional.prototype.selectNode = function(node) {

  var self = this;
  var oldSelection = node.selectionType;
  // With functional nodes, either unselected (REMOVED) or selected (EXPANED)
  var selections = epiviz.ui.charts.tree.HierarchyVisualization.ENUM_SELECTIONS;
  node.selectionType = oldSelection == selections['REMOVED'] ? selections['EXPANDED'] : selections['REMOVED'];
  Object.keys(self._selectedNodes).forEach(function(sel){
    if(sel == node.id){
      if(self._selectedNodes[sel] == 1 && node.selectionType == 0){
  ...
  if (node.selectionType == 1){
    self._selectedNodes[node.id] = node.selectionType;
  }
  console.log("in select Node");
  console.log(self._selectedNodes);
  this._changeNodeSelection(node, node.selectionType);
...
epiviz.ui.charts.tree.IcicleFunctional.prototype._changeNodeSelection = function(node, selectionType) {
  var self = this;
  var item = this._svg.select('#' + this.id() + '-' + node.id).select('rect');

  if(selectionType == epiviz.ui.charts.tree.HierarchyVisualization.ENUM_SELECTIONS['EXPANDED']){
  ...
  item = this._svg.select('#' + this.id() + '-' + node.id).select('span');
  console.log("_changeNodeSelection");
  console.log(node);
  console.log(selectionType);

  
  if (selectionType == 0){
      //item.classed('fa', false);
      item.classed('fa-minus', false);
      item.classed('fa', true);
      item.classed('fa-plus', true);
  }
  else if (selectionType == 1) {
      console.log("in selectionType = 1 block");
      //item.classed('fa', false);
      item.classed('fa-plus', false);
      item.classed('fa', true);
      item.classed('fa-minus', true);
  }
  console.log(item);
};