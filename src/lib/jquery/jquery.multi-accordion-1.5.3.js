...
(function($){
	...
	$.widget('ui.multiAccordion', {
      ...
      // menuOptions = self.menuOptions,
			...
			$this.children('div').each(function(index){
				var $this = $(this);
        // self._drawMenu(self, $this);
				$this.addClass(options._classes.div);
			}); // end each

      //$('.menuOptionsContainer ul').menu(); // Added another more efficient line in _drawMenu
        ...
        // self._showOptionsMenu(self, $this);
      ...
      $span.remove();
        ...
        /*
        case 'menuOptions':
          // If menu options change, redraw the menu
          var self = this;
          var $this = this.element;
          self.options.menuOptions = value;
          $this.children('div').each(function(index){
            var $this = $(this);
            self._drawMenu(self, $this);
          }); // end each
    ...
    _drawMenu:function(self, $this) {
      var options = self.options;
      var menuOptions = options.menuOptions;

      $this.next('.menuOptionsContainer').find('ul').menu('destroy');
      $this.next('.menuOptionsContainer').remove();

      var menu = '<span class="menuOptionsContainer"><ul></ul></span>';
      $this.after(menu);


      var menu = $this.next('.menuOptionsContainer');
      var ul = menu.children('ul');

      var optionFormat = '<li><a href="javascript:void(0);">%s</a></li>'
      var content = '';
      var keys = d3.keys(menuOptions);
      var li = null;
      for (var i = 0; i < keys.length; ++i) {
        li = sprintf(optionFormat, keys[i]);
        ul.append(li);
        li = ul.children('li').last();
        var callback = menuOptions[keys[i]];
        li.on('click', function() {
          callback.call($this);
          $('.menuOptionsContainer').css('visibility', 'hidden');
        });
      }
      li = sprintf(optionFormat, 'Toggle Expand/Collapse');
      ul.append(li);
      li = ul.children('li').last();
      ...
      li.bind('click', function() {
        if (h3.hasClass(options._classes.stateDefault)) {
          self._showTab(h3);
          $('.menuOptionsContainer').css('visibility', 'hidden');
        } else {
          self._hideTab(h3);
    ...
    _showOptionsMenu:function (self, $this) {
      var menu = $this.next('div').next('.menuOptionsContainer');
      var offset = $this.offset();
      menu.css('top', offset.top + 25);
      menu.css('left', offset.left + 5);
